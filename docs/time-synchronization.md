...
The additional ADIS16465 IMU provides higher-precision inertial measurements
than the MID360s built-in ICM40609, but it is not used as the ground-truth pose
reference. It is synchronized by timestamping its Data Ready (DR) signal with
the host local clock. The DR signal indicates that a new ADIS16465 sample is
ready. The DR signal time differs from the actual sampling time by about 1 ms.
...
- The host timestamp is used as the sample timestamp for the corresponding
  ADIS16465 measurement.
