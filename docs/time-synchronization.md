...
The additional ADIS16465 IMU is synchronized by timestamping its DR signal with
the host local clock. The DR signal time differs from the actual sampling time
by about 1 ms.
...
sensors. Its synchronization is based on the DR signal:

- The ADIS16465 outputs a DR signal for each data-ready event.
