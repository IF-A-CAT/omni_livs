...
timestamping its DR signal with the host local clock; the DR signal time differs
from the sampling time by about 1 ms. GNSS timestamps are converted from UTC or
GPS time generated by the receiver to UNIX time. Because the host receives time
over the network, GNSS position timestamps can have millisecond-level latency
